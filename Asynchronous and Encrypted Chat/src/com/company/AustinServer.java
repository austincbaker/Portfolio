...
public class AustinServer {
    ...
    public static void main(String[] args) throws IOException, NoSuchAlgorithmException, IllegalBlockSizeException, NoSuchPaddingException, BadPaddingException, InvalidKeyException {
        ...
        while (true)
        {
            ...
            String confirmPasswordRecipt= "";
            while(!confirmPasswordRecipt.equalsIgnoreCase("Received")){
                confirmPasswordRecipt = dis.readUTF();
            }
            //System.out.println("Confirmation received");
            dos.writeUTF("Please format the message in Your Message @Recipient Name" + "\n");
            dos.writeUTF("To send a message in the general chat, use @All Message" + "\n");
        ...
        while (true) {
            try {
                ...
                if (fromClient.length() > 0) {
                    ...
                    for (ClientList mc : AustinServer.clientListVector) {
                        ...
                        if (recipient.equalsIgnoreCase(lookingFor)) {
                            mc.dataOutputStream.writeUTF(this.name + " :@ " + fromClient);
                    ...
                    if (recipient.equalsIgnoreCase("all")) {
                        ...
                        for (ClientList mc2 : AustinServer.clientListVector) {
                            if (!this.name.equalsIgnoreCase(mc2.name)) {
                                ...
                                mc2.dataOutputStream.writeUTF(this.name + ":@ " + fromClient);
                                dataOutputStream.flush();
                            }
                        }
                    }
               // }
                ...
                try {
                    dataOutputStream.writeUTF("Unable to send. Please format the message in 'Recipient name' $ Message");
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
...
class ServerAes {
    ...
    public String encrypt(String unencryptedString, String secretString) {
        try {
            ...
            return Arrays.toString(cipher.doFinal(unencryptedString.getBytes(StandardCharsets.UTF_8)));
        } catch (Exception e) {
